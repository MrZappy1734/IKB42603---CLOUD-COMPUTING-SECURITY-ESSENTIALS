...
Symmetric encryption uses one shared secret key for both encryption and decryption. It is fast and suitable for large amounts of data, but the key must be exchanged securely. In the cloud, distributing and protecting the same shared key across users, services, regions, and tenants creates a key-distribution problem and increases the risk of exposure. Asymmetric encryption uses a public/private key pair. It is slower, but it is useful for secure key exchange, small messages, and digital signatures.

### Q2. Why are key-management failures more common than failures in the cryptographic algorithm?

Strong algorithms and mathematics are difficult to break when implemented correctly. Practical failures usually result from human or operational mistakes, such as hardcoded secrets, excessive permissions, exposed private keys, poor rotation practices, or incorrect storage and deletion procedures.

### Q3. Why does a master key require hardware-grade protection?

A master key protects many data keys. If it is compromised, every data key that it wrapped may also be exposed, creating a very large blast radius. Hardware-backed protection limits direct access to the master key and makes sensitive key operations more controlled and auditable.

### Q4. Why can cryptographic erasure be preferable to overwriting?

Cryptographic erasure destroys the key required to decrypt the data, making the encrypted data unusable even if residual copies remain. Overwriting may be incomplete on replicated, cached, or flash-based storage, so fragments can sometimes remain recoverable.

### Q5. How does hash chaining reveal tampering?

Changing an earlier log entry changes its hash. Because later entries depend on the previous hash, the changed value no longer matches the next link and the inconsistency propagates through the remainder of the chain.
