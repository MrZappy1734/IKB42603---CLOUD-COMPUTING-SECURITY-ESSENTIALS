...
## Task 1 - Environment Setup (LocalStack & AWS CLI)
...
A bucket policy that denies all requests not using HTTPS was applied to enforce encryption in transit:
...
A pre-signed URL was then generated for the `internal/roster.txt` object with a 60-second expiry to demonstrate time-limited access delegation without sharing credentials:
...
Versioned deletion was demonstrated by issuing a soft delete (placing a delete marker) and then recovering the original object:
...
**Answer:** Encryption at rest protects data while it is stored - in this lab, the KMS-managed server-side encryption (SSE-KMS) configured in Task 6 encrypted every object written to the bucket so that raw storage bytes are unreadable without the key. Encryption in transit protects data while it moves over the network - the HTTPS-only bucket policy in Task 7 (`DenyUnencryptedTransport`) ensures that any S3 API call not using TLS is denied, so the patient records are never sent in cleartext over the wire.

### Q2. Why does an explicit Deny in a bucket policy override an Allow in an IAM user policy?

**Answer:** AWS evaluates all applicable policies together and applies a strict Deny-wins rule: if any policy at any layer explicitly denies an action, that Deny overrides every Allow, regardless of where the Allow comes from. In Task 5, the DataAnalyst IAM policy allowed `s3:GetObject` on `*`, but the bucket policy's `DenyAnalystConfidential` statement explicitly denied all S3 actions on `confidential/*`. The Deny won, so the analyst could not access the confidential record even though the IAM policy permitted it.

### Q3. How does S3 versioning protect against accidental data loss, and what is its limitation?

**Answer:** When versioning is enabled, a `delete-object` call only places a delete marker as the current version; all previous versions are retained and can be recovered by specifying a `--version-id`. In Task 9, after the soft delete the original record was recovered by fetching `--version-id null`. The limitation is cost: every version consumes storage, and the bucket can accumulate many copies. Lifecycle rules (Task 10) are needed to automatically expire non-current versions and keep storage costs under control.
...
**Answer:** Least privilege means granting only the minimum permissions required for a task, and no more. In this lab it was applied at two layers: (1) the bucket policy in Task 4 was narrowed from allowing anonymous reads of every object to allowing only the named account to read objects in the `internal/` prefix; (2) in Task 5, the analyst's IAM policy was broad, but the bucket policy added an explicit Deny on `confidential/*`, so the analyst could only access the data their role required (`internal/`) and nothing more sensitive.

## Cleanup & Teardown

The LocalStack container and locally created files were removed after evidence was captured:

```bash
aws $EP s3api delete-bucket-policy --bucket $BUCKET
aws $EP kms cancel-key-deletion --key-id $KEY_ID
# Remove all objects and delete the bucket
aws $EP s3 rb s3://$BUCKET --force
docker stop localstack && docker rm localstack
rm -f public-notice.txt internal-roster.txt confidential-record.txt \
      public-policy.json least-privilege-policy.json deny-confidential.json \
      analyst-iam.json encryption.json secure-transport.json lifecycle.json \
      leaked.txt rec-v2.txt rec-v3.txt gone.txt recovered.txt after-erasure.txt
```
